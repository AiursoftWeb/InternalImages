...
| `ASR_MAX_STORED_AUDIO_SIZE_MIB` | 正在上传、排队中和运行中任务的临时音频文件总容量上限（MiB），范围为 1–1048576。达到上限时，新任务返回 `429`。 | `2048` |
...
> - WhisperX 模型服务本身不维护任务队列；已有任务运行时，直接请求模型服务会返回 `409 Conflict`。生产调用应统一经过 `asr-api`，由网关负责排队和取消。
...
该接口只返回网关和上游模型服务的健康状态。音频分片策略完全由调用方配置和执行，不由网关下发。单个任务执行超时后，网关会取消上游推理、清理任务资源并继续处理同模型队列中的下一个任务。
...
使用 `POST /v1/tasks/:id/cancel` 可以取消排队中或运行中的任务。若任务尚未上传，网关会短期保留该任务 ID 的取消记录并拒绝随后到达的同 ID 任务。运行中任务取消后会立即丢弃识别结果；接口仅在上游推理已停止且网关临时文件、存储额度等本地资源完成清理后返回成功。

单个音频文件最大为 1 GiB。网关会为 multipart 边界、文件名和表单字段额外预留 1 MiB 请求空间；该空间不计入音频文件大小上限。长音频的切片、重叠和结果合并由调用方负责。
